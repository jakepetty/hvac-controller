...
class GenericThermostat(ClimateDevice, RestoreEntity):
    ...
    def __init__(self, hass, name, heater_entity_id, cooler_entity_id, sensor_entity_id,
        ...
        if self._keep_alive:
            ...
            async_track_time_interval(
                hass, self._async_control_cooling, self._keep_alive)
    ...
    def state(self):
        ...
        if self._is_heat_device_active:
            return self.current_operation
        if self._is_cool_device_active:
            return self.current_operation
        if self._enabled:
            return STATE_IDLE
        return STATE_OFF
    ...
    async def _async_control_cooling(self, time=None, force=False):
        ...
        async with self._temp_lock:
            ...
            if self._is_cool_device_active:
                ...
                if (too_hot):
                    _LOGGER.info("Turning on heater %s", self.cooler_entity_id)
                    await self._async_cooler_turn_on()
                elif time is not None:
                    # The time argument is passed only in keep-alive case
                    await self._async_cooler_turn_off()
    ...
    @property
    def _is_heat_device_active(self):
        """If the toggleable device is currently active."""
        return self.hass.states.is_state(self.heater_entity_id, STATE_ON)

    @property
    def _is_cool_device_active(self):
        """If the toggleable device is currently active."""
